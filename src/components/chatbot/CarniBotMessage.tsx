...
const CarniBotMessage = memo(({ message }: { message: Message }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex items-start gap-2 ${isBot ? "" : "flex-row-reverse"}`}>
      {isBot && (
        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-primary to-purple-600 flex items-center justify-center text-xs shrink-0">🤖</div>
      )}
      <div className={`max-w-[80%] ${isBot ? "" : "ml-auto"}`}>
        <div className={isBot ? "glass rounded-2xl rounded-tl-sm px-4 py-3 border border-primary/20" : "bg-gradient-to-br from-primary to-purple-600 text-white rounded-2xl rounded-tr-sm px-4 py-3"}>
